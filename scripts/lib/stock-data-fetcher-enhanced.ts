...
async function fetchFromPolygon(symbol: string): Promise<StockData | null> {
  try {
    ...
    const previousClose = history.length > 0 ? history[history.length - 1].close : price;
    const change = price - previousClose;
    const changePercent = previousClose ? (change / previousClose) * 100 : 0;

    const recentVolumes = history.slice(-50).map((h: any) => h.volume).filter((v: number) => v > 0);
    const avgVolume = recentVolumes.length > 0
      ? recentVolumes.reduce((a: number, b: number) => a + b, 0) / recentVolumes.length
      : volume;
  ...
  try {
    ...
    if (historyResponse.ok) {
      ...
      if (historyData.values && Array.isArray(historyData.values)) {
        history = historyData.values.map((v: any) => ({
        ...
        const recentVolumes = history.slice(-50).map((h: any) => h.volume).filter((v: number) => v > 0);
        avgVolume = recentVolumes.length > 0
          ? recentVolumes.reduce((a: number, b: number) => a + b, 0) / recentVolumes.length
          : 0;
      }
    }
    ...
    if (profileResponse.ok) {
      ...
      if (profileData.name) {
        name = profileData.name;
        marketCap = profileData.market_cap ? parseFloat(profileData.market_cap) : undefined;
      }
    }
