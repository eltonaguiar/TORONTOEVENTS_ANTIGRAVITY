...
import { fetchMultipleStocks as fetchBasic } from './lib/stock-data-fetcher';
import { fetchMultipleStocks as fetchEnhanced } from './lib/stock-data-fetcher-enhanced';
...
async function fetchStocks(symbols: string[]) {
  try {
    return await fetchEnhanced(symbols);
  } catch {
    return await fetchBasic(symbols);
  }
}

async function main() {
  const [symbol = '', algorithm = '', timeframe = '7d'] = process.argv.slice(2).map(s => String(s).trim());
  if (!symbol || !algorithm) {
    console.log(JSON.stringify({ error: 'Usage: score-one SYMBOL ALGORITHM [TIMEFRAME]' }));
    process.exit(1);
  }
  ...
  try {
    const dataList = await fetchStocks([sym]);
    const data = dataList.find(d => d.symbol.toUpperCase() === sym) ?? dataList[0];
    if (!data) {
    ...
    if (algo.includes('can slim') || algo === 'canslim') {
      ...
      console.log(JSON.stringify({ error: `Unknown algorithm: ${algorithm}. Use "CAN SLIM", "Technical Momentum", or "Composite Rating"` }));
      process.exit(1);
    }

    if (!result) {
      console.log(JSON.stringify({ error: `Algorithm returned no score for ${sym} (e.g. insufficient history)` }));
      process.exit(1);
    }
