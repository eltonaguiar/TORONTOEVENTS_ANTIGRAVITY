...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(archivePath, JSON.stringify({ lastUpdated, totalPicks: stampedStocks.length, stocks: stampedStocks }, null, 2));
    ...
    console.log(`  • STRONG BUY: ${stampedStocks.filter(s => s.rating === 'STRONG BUY').length}`);
    console.log(`  • BUY: ${stampedStocks.filter(s => s.rating === 'BUY').length}`);
    console.log(`  • HOLD: ${stampedStocks.filter(s => s.rating === 'HOLD').length}`);
